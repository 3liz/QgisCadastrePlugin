...
-- DELETE FROM [PREFIXE]geo_tsurf USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_tsurf.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_numvoie USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_numvoie.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_voiep USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_voiep.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_lieudit USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(st_buffer(geo_lieudit.geom,-1),c.geom);
-- DELETE FROM [PREFIXE]geo_zoncommuni USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_zoncommuni.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_tpoint USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_tpoint.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_tline USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_tline.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_tronfluv USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_tronfluv.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_symblim USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_symblim.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_croix USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_croix.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_borne USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_borne.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_ptcanv USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_ptcanv.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_subdfisc USING (SELECT DISTINCT geo_subdfisc FROM [PREFIXE]geo_subdfisc_parcelle p, [PREFIXE]impedigeo_commune_id c WHERE SUBSTRING(geo_parcelle,1,7) = '[ANNEE]'||c.idu) lst WHERE geo_subdfisc.geo_subdfisc=lst.geo_subdfisc;
-- DELETE FROM [PREFIXE]geo_batiment USING (SELECT DISTINCT geo_batiment FROM [PREFIXE]geo_batiment_parcelle p, [PREFIXE]impedigeo_commune_id c WHERE SUBSTRING(geo_parcelle,1,7) = '[ANNEE]'||c.idu) lst WHERE geo_batiment.geo_batiment=lst.geo_batiment;
-- DELETE FROM [PREFIXE]geo_batiment USING (SELECT DISTINCT geom FROM [PREFIXE]impedigeo_commune_id) c WHERE st_intersects(geo_batiment.geom,c.geom);
-- DELETE FROM [PREFIXE]geo_commune USING (SELECT DISTINCT idu FROM [PREFIXE]impedigeo_commune_id) lst WHERE annee='[ANNEE]'and geo_commune.idu=lst.idu;
