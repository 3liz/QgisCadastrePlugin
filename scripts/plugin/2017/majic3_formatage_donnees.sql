...
SELECT
  ...
  CASE WHEN trim(SUBSTRING(tmp,38,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,40,6),'DDMMYYYY')+(SUBSTRING(tmp,38,2)::integer - 1 || ' day')::interval END AS jdatat,
...
SELECT
  ...
  CASE WHEN trim(SUBSTRING(tmp,139,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,141,6),'DDMMYYYY')+(SUBSTRING(tmp,139,2)::integer - 1 || ' day')::interval END AS datja,
  ...
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,15),' ', '0') AS parcelle,
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,2)||SUBSTRING(tmp,4,3)||SUBSTRING(tmp,31,6),' ', '0') AS comptecommunal,
...
SELECT
  ...
  CASE WHEN trim(SUBSTRING(tmp,44,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,46,6),'DDMMYYYY')+(SUBSTRING(tmp,44,2)::integer - 1 || ' day')::interval END AS jdatat,
  ...
  CASE WHEN trim(SUBSTRING(tmp,119,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,121,6),'DDMMYYYY')+(SUBSTRING(tmp,119,2)::integer - 1 || ' day')::interval END AS jdatcgl,
  ...
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,2)||SUBSTRING(tmp,4,3)||SUBSTRING(tmp,38,6),' ', '0') AS comptecommunal,
  '[LOT]' as lot
...
SELECT DISTINCT ON (ccodep,ccocom,dnupro,dnulp,dnuper)
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,2)||SUBSTRING(tmp,7,6)||SUBSTRING(tmp,13,2)||SUBSTRING(tmp,19,6), ' ', '0') AS proprietaire,
  '[ANNEE]' AS annee,
  SUBSTRING(tmp,1,2) AS ccodep,
  SUBSTRING(tmp,3,1) AS ccodir,
  SUBSTRING(tmp,4,3) AS ccocom,
  ...
  CASE WHEN trim(SUBSTRING(tmp,335,10))='' THEN NULL WHEN SUBSTRING(tmp,335,10)='00/00/0000' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,337,8),'DD/MM/YYYY')+(SUBSTRING(tmp,335,2)::integer - 1 || ' day')::interval END AS jdatnss,
  ...
  CASE WHEN trim(SUBSTRING(tmp,479,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,481,6),'DDMMYYYY')+(SUBSTRING(tmp,479,2)::integer - 1 || ' day')::interval END AS datja,
  ...
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,2)||SUBSTRING(tmp,4,3)||SUBSTRING(tmp,7,6),' ', '0') AS comptecommunal,
  '[LOT]' as lot
FROM [PREFIXE]prop
...
SELECT
  REPLACE('[ANNEE]'||ccodep||ccocom||dnupro,' ', '0') AS comptecommunal,
  '[ANNEE]',
  ccodep,
  ccodir,
  ccocom,
...
SELECT
  ...
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,15),' ', '0') AS parcelle,
  REPLACE('[ANNEE]'||SUBSTRING(tmp,1,2)||SUBSTRING(tmp,4,3)||SUBSTRING(tmp,83,6),' ', '0') AS comptecommunal,
  '[LOT]' as lot
...
SELECT
  ...
  CASE WHEN trim(SUBSTRING(tmp,72,8))='' THEN NULL ELSE to_date('01' || SUBSTRING(tmp,74,6),'DDMMYYYY')+(SUBSTRING(tmp,72,2)::integer - 1 || ' day')::interval END AS datact,
