...
(
 ...
 dparpi, ccoarp, gparnf, gparbat, parrev, gpardp, fviti, dnvoiri, dindic, ccovoi, ccoriv, ccocif, ccpper, gpafpd, ajoutcoherence,
...
(
 sufexoneration, annee, ccodep, ccodir, ccocom, ccopre, ccosec, dnupla, ccosub, rnuexn, ccolloc, pexn, gnexts, jandeb, jfinex, fcexn, fcexna, rcexna,
...
SELECT DISTINCT
  ...
  CASE WHEN trim(SUBSTRING(tmp,168,11))='' THEN NULL ELSE to_number(SUBSTRING(tmp,168,11),'99999999999') END AS mlbtfc,
...
SELECT DISTINCT ON (ccodep,ccocom,dnupro,dnulp,dnuper)
  ...
  CASE WHEN SUBSTRING(tmp,151,5) ~ '[0-9]{4}[A-Z]' THEN CASE WHEN (SUBSTRING(tmp,151,10) ~ '[0-9]{4}[A-Z]{6}' AND SUBSTRING(tmp,156,4) IN (SELECT natvoi FROM ${PREFIXE}natvoi)) THEN SUBSTRING(tmp,151,4) || ' ' || SUBSTRING(tmp,155,1) || ' ' || SUBSTRING(tmp,156,4) || ' ' || SUBSTRING(tmp,160,27) ELSE SUBSTRING(tmp,151,4) || ' ' || SUBSTRING(tmp,155,1) || ' ' || SUBSTRING(tmp,156,31) END ELSE CASE WHEN (SUBSTRING(tmp,151,10) ~ '[0-9]{4}.[A-Z]{5}' AND SUBSTRING(tmp,156,4) IN (SELECT natvoi FROM ${PREFIXE}natvoi)) THEN SUBSTRING(tmp,151,9) || ' ' || SUBSTRING(tmp,160,27) ELSE SUBSTRING(tmp,151,36) END END AS dlign4,
...
SELECT
  '${DEPDIR}' || code_commune AS commune,
  '${DEPDIR}' || code_commune AS geo_commune,
  '${ANNEE}' AS annee,
  code_dep AS ccodep,
  SUBSTRING('${DEPDIR}', 3, 1) AS ccodir,
  code_commune AS ccocom,
  NULL AS clerivili,
  ...
  NULL AS motclas,
  '${LOT}' as lot
FROM ${PREFIXE}topo WHERE code_type_topo = '13'
...
SELECT
  REPLACE('${DEPDIR}' || code_commune || '00000' || code_voie , ' ', '0')  AS voie,
  '${ANNEE}',
  code_dep AS ccodep,
  SUBSTRING('${DEPDIR}', 3, 1) AS ccodir,
  code_commune AS ccocom,
  SUBSTRING(code_voie, 1, 1) AS natvoiriv,
  code_voie AS ccoriv,
  NULL AS clerivili,
